export default {
  'game': {
    'bossBanner': 'Босс кезеңі!',
    'saiyan': {
      'verb': 'Ааааргх!',
      'hint': 'Қуат деңгейін көтеру үшін басыңыз',
      'pump': 'Айда!',
      'over': '9000-нан асты!'
    },
    'cat': {
      'verb': 'Сипа!',
      'hint': 'Сақина жабылғанда әр ұшқынды басыңыз',
      'zap': 'Шырт!'
    },
    'window': {
      'verb': 'Жап!',
      'hint': 'Тұтқаны тартып тарс еткіз',
      'slam': 'Тарс!'
    },
    'flashlight': {
      'verb': 'Ұр!',
      'hint': 'Ырғақпен үш рет ұр',
      'whack': 'Шап!'
    },
    'toast': {
      'verb': 'Ұста!',
      'hint': 'Екі тілімді де ауада ұста'
    },
    'soda': {
      'verb': 'Түрт!',
      'hint': '5 рет түрт, содан кейін қақпағын аш',
      'spray': 'Шыпыр!'
    },
    'router': {
      'verb': 'Қос!',
      'hint': 'Ашаны розеткаға сүйреңіз'
    },
    'mosquito': {
      'verb': 'Шапалақта!',
      'hint': 'Шеңбер жабылғанда дәл басыңыз',
      'smack': 'Шап!'
    },
    'stomp': {
      'verb': 'Басып жанш!',
      'hint': 'Уақыт бітпей тұрып әр жәндікті жаныш',
      'splat': 'Шып!'
    },
    'mirage': {
      'verb': 'Соқ!',
      'hint': 'ШЫНАЙЫ қосаяқты соқ, сағымды емес',
      'boop': 'Боп!'
    },
    'thermometer': {
      'verb': 'Суыт!',
      'hint': 'Сақиналар жабылғанда мұздарды кезекпен бас',
      'boiling': 'Қайнау нүктесі'
    },
    'potato': {
      'verb': 'Ла!',
      'hint': 'Күймес бұрын лақтыру үшін бос қолды бас',
      'ouch': 'Ай!'
    },
    'fan': {
      'verb': 'Желпі!',
      'hint': 'Ыстықты қуу үшін ырғақпен бас',
      'stress': 'Ыстық'
    },
    'shadow': {
      'verb': 'Секір!',
      'hint': 'Ыстық құбырдан секіру үшін көлеңкені бас',
      'phew': 'Уһ!',
      'burn': 'Ыстық!'
    },
    'signal': {
      'verb': 'Баптаңыз!',
      'hint': 'Сигнал табылғанша антеннаны бұраңыз',
      'tune': 'Іздеу',
      'hold': 'Ұста!'
    }
  },
  'desk': {
    'prompt': 'Түс көру үшін бос орын'
  },
  'summary': {
    'morning': 'Таң!',
    'lightsOut': 'Шам сөнді.',
    'ideas': 'түнгі 3-тегі идея жүзеге асты',
    'best': 'Рекорд:',
    'newBest': 'Жаңа рекорд!',
    'again': 'Тағы бір түн?'
  },
  'ads': {
    'watch': 'Көру', 'revive': 'Тірілту', 'secondChance': 'Екінші мүмкіндік', 'doubleCoins': '2× тиын', 'plusCoins': '+{n} тиын'
  },
  'achievements': {
    'title': 'Жетістіктер', 'subtitle': 'Тиын табу үшін мақсаттарға жетіңіз.', 'claim': 'Алу', 'claimed': 'Алынды', 'progress': '{c} / {t}',
    'items': {
      'tiles1k': { 'name': 'Әлем кезуші', 'desc': 'Барлығы 1 000 тақтайшадан өтіңіз.' },
      'tiles5k': { 'name': 'Марафоншы', 'desc': 'Барлығы 5 000 тақтайшадан өтіңіз.' },
      'tiles10k': { 'name': 'Алыс қашықтыққа жүгіруші', 'desc': 'Барлығы 10 000 тақтайшадан өтіңіз.' },
      'tiles100k': { 'name': 'Саяхатшы', 'desc': 'Барлығы 100 000 тақтайшадан өтіңіз.' },
      'stage5': { 'name': 'Алғашқы қадамдар', 'desc': '5-кезеңге жетіңіз.' },
      'stage10': { 'name': 'Тәжірибелі', 'desc': '10-кезеңге жетіңіз.' },
      'stage20': { 'name': 'Ардагер', 'desc': '20-кезеңге жетіңіз.' },
      'clears25': { 'name': 'Тазалаушы', 'desc': 'Барлығы 25 кезеңді өтіңіз.' },
      'clears100': { 'name': 'Центурион', 'desc': 'Барлығы 100 кезеңді өтіңіз.' },
      'bestRun100': { 'name': 'Спринтер', 'desc': 'Бір жүгірісте 100 тақтайшадан өтіңіз.' },
      'bestRun250': { 'name': 'Қашықтық жыны', 'desc': 'Бір жүгірісте 250 тақтайшадан өтіңіз.' },
      'coins5k': { 'name': 'Тиын жинаушы', 'desc': 'Барлығы 5 000 тиын жинаңыз.' },
      'coins50k': { 'name': 'Қазынашы', 'desc': 'Барлығы 50 000 тиын жинаңыз.' },
      'items50': { 'name': 'Ашушы', 'desc': 'Барлығы 50 зат жәшігін алыңыз.' },
      'items250': { 'name': 'Жинаушы', 'desc': 'Барлығы 250 зат жәшігін алыңыз.' }
    }
  },
  'gameName': 'Midnight Analog',
  'cancel': 'Бас тарту',
  'close': 'Жабу',
  'ok': 'Жарайды',
  'continue': 'Жалғастыру',
  'tapToContinue': 'Жалғастыру үшін түртіңіз',
  'clickToContinue': 'Жалғастыру үшін басыңыз',
  'stage': 'Кезең',
  'rewards': 'СЫЙЛЫҚТАР',
  'tip': 'Кеңес',
  'crazyGamesOnly': 'Бұл ойын тек мына жерде қолжетімді',
  'startTouch': 'Бастау үшін түртіңіз',
  'startDesktop': 'Бастау үшін басыңыз',
  'startSubhint': 'Жоғары қарай домалаңыз — саңылаулар мен кедергілерден жалтару үшін бағытты өзгертіңіз!',
  'hints': { 'tapToTurn': 'Бағытты өзгерту үшін түртіңіз', 'clickToTurn': 'Бағытты өзгерту үшін басыңыз / Бос орын' },
  'powerups': { 'invuln': 'Жеңілмейтін', 'magnet': 'Монета магниті', 'dodge': 'Жалтару шебері', 'slowmo': 'Баяу қозғалыс', 'push': 'Итеру күші',  'racer': 'Жарысшы!' },
  'secondChance': { 'title': 'Домалауды жалғастырасыз ба?', 'body': 'Қайта тіріліп, жүгірісіңізді жалғастыру үшін қысқа жарнаманы көріңіз.', 'watch': 'Көру және жалғастыру', 'skip': 'Өткізіп жіберу' },
  'result': { 'win': 'КЕЗЕҢ ӨТТІ!', 'lose': 'ОЙЫН АЯҚТАЛДЫ', 'fell': 'Сіз саңылауға құлап түстіңіз!', 'crashed': 'Сіз кедергіге соғылдыңыз!', 'tiles': 'Тақтайшалар', 'winReward': 'қоса +{n} кезең бонусы', 'double': 'Қос монета', 'firstRunDouble': '2× — бүгінгі алғашқы ойын!',  'almost': 'Аз қалды! Тағы {n} тор.', 'retry': 'Қайталау', },
  'upgrades': {
    'title': 'Жетілдірулер',
    'subtitle': 'Тұрақты күшею үшін монеталарды жұмсаңыз.',
    'level': '{n}-деңг.',
    'maxedOut': 'ШЕКТІ',
    'sellBack': 'Сату +{n}',
    'spotlight': 'Жұмсаңыз!',
    'unlocksAtStage': '🔒 {n}-кезең',
    'names': { 'powerupDuration': 'Қуат күшейткіші', 'magnetRange': 'Магнит ауқымы', 'coinValue': 'Монета құны', 'itemLuck': 'Сәтті жәшіктер', 'dodgeApprentice': 'Жалтару шәкірті', 'deathMagnet': 'Өлім магниті', 'autoCollect': 'Авто-жинау', 'rollingBoulder': 'Домалаған тас' },
    'descriptions': { 'powerupDuration': 'Барлық қуат күшейткіштер ұзағырақ әсер етеді (деңгейіне +0,75 сек).', 'magnetRange': 'Монета магниті алысырақ жетеді.', 'coinValue': 'Әрбір жинаған монетаңыз қымбатырақ.', 'itemLuck': 'Зат жәшіктері жиірек пайда болады.', 'dodgeApprentice': 'Бір өлімші тақтайшадан автоматты түрде жалтарып, содан кейін зарядталады (10 сек; деңгейіне −0,5 сек).', 'deathMagnet': 'Өлген кезде 4 тақтайша аралығындағы барлық монеталарды жинаңыз — жоғалтатын монеталарыңызды сақтап қалыңыз.', 'autoCollect': 'Жақын тақтайшалардан монеталарды әрқашан жинаңыз — тұрақты 1 тақтайшалық магнит.', 'rollingBoulder': 'Жәшік кедергілерінен зақым алмай тура домалаңыз.' },
    'secondChance': { 'name': 'Екінші мүмкіндік', 'description': 'Әрбір жүгірісті періште қанаттарымен бастаңыз — бір соқтығысуға немесе құлауға төтеп беріңіз. Қолданғанша белсенді болады.', 'active': 'БЕЛСЕНДІ', 'watch': 'Тегін' }
  },
  'battlePass': { 'title': 'Шайқас өткелі', 'progress': '{current} / {total}', 'daysLeft': '{n} күн қалды', 'maxed': 'ШАЙҚАС ӨТКЕЛІ АЯҚТАЛДЫ', 'xpProgress': '{current} / {total} ТТ', 'howToEarn': 'ТТ қалай табуға болады', 'perAttempt': 'жүгіріс сайын', 'perStageFinish': 'кезең өткені үшін', 'unlockHint': 'Келесі сыйлықты ашу үшін {n} ТТ-ға жетіңіз — алынбаған сыйлықтар оларды түрткенге дейін сақталады.' },
  'dailyRewards': { 'title': 'Күнделікті сыйлықтар', 'subtitle': 'Сериямызды сақтау үшін күн сайын кіріңіз.', 'day': '{n}-күн', 'dayShort': '{n}-к' },
  'skins': {
    'title': 'Доп Скиндері',
    'subtitle': 'Жаңа сыртқы түрлерді ашу және кию үшін монета жұмсаңыз.',
    'equip': 'Кию',
    'equipped': 'Киілген',
   'locked': 'Кезең {n}', 'new': 'Жаңа!', 'rarity': { 'common': 'Қарапайым', 'rare': 'Сирек', 'epic': 'Эпик' }, },
  'missions': {
    'title': 'Күнделікті тапсырмалар',
    'subtitle': 'Монета алу үшін күн сайын мақсаттарды орындаңыз.',
    'claim': 'Алу',
    'done': 'Алынды',
    'types': {
      'coins': 'Бүгін {n} монета жинаңыз',
      'tiles': 'Бір ойында {n} тақтайшадан өтіңіз',
      'items': 'Бүгін {n} зат жәшігін алыңыз',
      'clears': 'Бүгін {n} кезеңді өтіңіз'
    }
  },
  'endless': {
    'badge': 'Шексіз',
    'toEndless': 'Шексіз режим',
    'toCampaign': 'Науқан',
    'best': 'Үздік: {n}'
  },
  'boon': {
    'title': 'Сыйлықты таңдаңыз',
    'names': {
      'secondChance': 'Екінші мүмкіндік',
      'startPowerup': 'Жылдам бастау',
      'coinBoost': 'Монета тасқыны'
    },
    'descriptions': {
      'secondChance': 'Келесі кезеңді Екінші мүмкіндік қалқанымен бастаңыз.',
      'startPowerup': 'Келесі кезеңді кездейсоқ қуат күшейткішімен бастаңыз.',
      'coinBoost': 'Бүкіл келесі кезеңге 1,2× монета.'
    }
  },
  'options': {
    'title': 'Параметрлер', 'general': 'Жалпы', 'audio': 'Дыбыс', 'language': 'Тіл', 'difficulty': 'Қиындық', 'soundEffects': 'Дыбыс әсерлері', 'music': 'Музыка', 'musicTrack': 'Музыка тректі', 'musicTracks': { 'cozy': 'Жайлы әуен', 'trance': 'Транс туннелі' }, 'close': 'Сақтау және жабу',
    'difficulties': { 'easy': 'Оңай', 'medium': 'Орташа', 'hard': 'Қиын' },
    'difficultyHints': { 'easy': 'Баяу жылдамдық — әрекет етуге көбірек уақыт.', 'medium': 'Стандартты, теңдестірілген қарқын.', 'hard': 'Жоғары жылдамдық — қатаңырақ, дәлірек уақыт.' }
  },
  'adsBlocked': {
    'title': 'Жарнаманы көрсету мүмкін болмады',
    'body': 'Біз сізге сыйлық алу үшін бейне көрсетпек болдық, бірақ браузеріңіздегі бірдеңе жарнаманы бөгеп тұр.',
    'allowPrefix': 'Жарнамаға рұқсат беріңіз:',
    'allowSuffix': '(немесе осы ойын үшін жарнама бөгеушіні уақытша өшіріңіз) және қайталап көріңіз.',
    'gotIt': 'Түсінікті'
  },
  'saveStatus': {
    'restoredTitle': 'Бұлттық сақтау қалпына келтірілді',
    'restoredBody': 'Қалпына келтіру үшін +{n} бонустық тиын',
    'tap': 'түртіңіз',
    'pausedTitle': 'Бұлттық синхрондау кідіртілді',
    'pausedBody': 'Желіден тыс ойнап жатырсыз. Прогресіңіз осында сақталады.',
    'retry': 'Қайталау',
    'dismiss': 'жабу'
  },
  'loading': {
    'tooLong': 'Жүктеу тым ұзаққа созылып жатыр ма? Жарнама бөгеушіні өшіріп, бетті жаңартыңыз.'
  },
  'license': {
    'denied': 'Кіруге тыйым салынды: лицензия сатып алыңыз.'
  }
}
